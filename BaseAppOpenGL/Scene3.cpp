...
int CScene3::DrawGLScene(void)	// Função que desenha a cena
{
	...
	for (int i = 0; i < 38; i++)
	{
		int p1 = index_vertices[i].p1;
		int p2 = index_vertices[i].p2;
		int p3 = index_vertices[i].p3;
		int p4 = index_vertices[i].p4;

		glColor3ub(faceColor[i].R, faceColor[i].G, faceColor[i].B);

		glVertex3f(vertices[p1].x, vertices[p1].y, vertices[p1].z);
		glVertex3f(vertices[p2].x, vertices[p2].y, vertices[p2].z);
		glVertex3f(vertices[p3].x, vertices[p3].y, vertices[p3].z);
		glVertex3f(vertices[p4].x, vertices[p4].y, vertices[p4].z);
	}
	glEnd();
	glPopMatrix();
